"""Generate or execute a one-command Linux upgrade flow for OmniBot v3."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run or print the OmniBot Linux upgrade plan.")
...
def main() -> int:
    ...
    plan = build_linux_upgrade_plan(config)

    if not args.execute:
        print(render_linux_upgrade_report(plan))
        return 0

    for step in plan.steps:
        print(f"[upgrade] {step.name}: {step.description}")
